...
from registration import est_lin_transf, est_nl_transf, est_initial_transf
...
def seg_atlas(im, atlas_ct_list, atlas_seg_list):
    ...
    for i in range(len(atlas_ct_list)):
        ...
        # estimate and apply initial transform
        print("Register image", i + 1, " out of ", len(atlas_ct_list), "... Estimate inital registration")
        transf_init = est_initial_transf(im, im_atlas)
        im_atlas_init = apply_transf(im, im_atlas, transf_init)

        # estimate and apply linear transform
        print("Register image", i + 1, " out of ", len(atlas_ct_list), "... Estimate linear registration")
        transf_lin = est_lin_transf(im, im_atlas_init)
        im_atlas_lin = apply_transf(im, im_atlas_init, transf_lin)
        ...
        segmentation = apply_transf(im, mask_atlas, transf_init)
        segmentation = apply_transf(im, segmentation, transf_lin)
